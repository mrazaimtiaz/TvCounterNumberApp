...
class GetEmployeeSignature @Inject constructor(
    private val repository: MyRepository
) {
    operator fun invoke(getPersonSendModel: GetPersonSendModel): Flow<Resource<List<EmployeeSignature>?>> = flow {
        try {
            emit(Resource.Loading())

            var result = repository.getEmployeeSignature(getPersonSendModel)
